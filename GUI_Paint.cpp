...
UWORD DarkenColor(UWORD color, float transparency) {
  ...
  float alpha = (100.0f - transparency) / 100.0f;  // Convert transparency to a float between 0 and 1
...
UWORD Paint_GetPixel(UWORD Xpoint, UWORD Ypoint) {
  if (Xpoint > Paint.Width || Ypoint > Paint.Height) {
    Debug("Exceeding display boundaries\r\n");
    return 0;  // Return default color (black) if out of bounds
  }
  UWORD X, Y;
  ...
  switch (Paint.Rotate) {
    ...
    case 270:
      ...
      break;
    default:
      return 0;  // Return default color (black) if rotation not supported
  }
  ...
  switch (Paint.Mirror) {
    ...
    case MIRROR_ORIGIN:
      ...
      break;
    default:
      return 0;  // Return default color (black) if mirror not supported
  }
  ...
  if (Paint.Scale == 2) {
    ...
    UBYTE Rdata = Paint.Image[Addr];
    return (Rdata & (0x80 >> (X % 8))) ? BLACK : WHITE;
  } else if (Paint.Scale == 4) {
    Addr = X / 4 + Y * Paint.WidthByte;
    UBYTE Rdata = Paint.Image[Addr];
    return (Rdata >> ((X % 4) * 2)) & 0x03;  // Extract color bits
  } else if (Paint.Scale == 16) {
    Addr = X / 2 + Y * Paint.WidthByte;
    UBYTE Rdata = Paint.Image[Addr];
    return (Rdata >> ((X % 2) * 4)) & 0x0F;  // Extract color bits
  } else if (Paint.Scale == 65) {
    Addr = X * 2 + Y * Paint.WidthByte;
    return (Paint.Image[Addr] << 8) | Paint.Image[Addr + 1];
  }

  return 0;  // Return default color (black) if scale not supported
}


void Paint_DrawRectangleTrans(UWORD Xstart, UWORD Ystart, UWORD Xend, UWORD Yend,
                              uint8_t transparency, DOT_PIXEL Line_width, DRAW_FILL Draw_Fill) {
  if (Xstart > Paint.Width || Ystart > Paint.Height || Xend > Paint.Width || Yend > Paint.Height) {
  ...
  if (Draw_Fill) {
    ...
    for (Ypoint = Ystart; Ypoint < Yend; Ypoint++) {
      ...
      for (Xpoint = Xstart; Xpoint < Xend; Xpoint++) {
        ...
        // Darken the color based on transparency
        UWORD newColor = DarkenColor(existingColor, transparency);
    ...
    Paint_DrawLine(Xstart, Ystart, Xend, Ystart, BLACK, Line_width, LINE_STYLE_SOLID);
    Paint_DrawLine(Xstart, Ystart, Xstart, Yend, BLACK, Line_width, LINE_STYLE_SOLID);
    Paint_DrawLine(Xend, Yend, Xend, Ystart, BLACK, Line_width, LINE_STYLE_SOLID);
    Paint_DrawLine(Xend, Yend, Xstart, Yend, BLACK, Line_width, LINE_STYLE_SOLID);
  }
}


void Paint_DrawCircleTrans(UWORD Xcenter, UWORD Ycenter, UWORD Radius,
                           uint8_t transparency, DOT_PIXEL Line_width, DRAW_FILL Draw_Fill) {
  // Calculate the boundaries of the circle
  UWORD Xstart = Xcenter - Radius;
  UWORD Ystart = Ycenter - Radius;
  UWORD Xend = Xcenter + Radius;
  UWORD Yend = Ycenter + Radius;

  // Ensure the boundaries are within the display range
  if (Xstart > Paint.Width || Ystart > Paint.Height || Xend > Paint.Width || Yend > Paint.Height) {
    Debug("Input exceeds the normal display range\r\n");
    return;
  }

  // Iterate through each pixel within the circle's bounding box
  UWORD Ypoint;
  for (Ypoint = Ystart; Ypoint <= Yend; Ypoint++) {
    UWORD Xpoint;
    for (Xpoint = Xstart; Xpoint <= Xend; Xpoint++) {
      ...
      // Check if the pixel is within the radius of the circle
...
void Paint_DrawImage1(const unsigned char pattern[], UWORD Xstart, UWORD Ystart, UWORD Xend, UWORD Yend, UWORD Color_Foreground) {
  ...
  for (Y = Ystart; Y < Ystart + Yend; Y++) {
    for (X = Xstart; X < Xstart + Xend; X++) {
      // Extract the bit corresponding to the current pixel from the pattern
      unsigned char pixelValue = (pattern[patternIndex / 8] >> (7 - (patternIndex % 8))) & 1;

      // Set the pixel based on the extracted value
      if (pixelValue != 0) {
        Paint_SetPixel(X, Y, Color_Foreground);
      }
...
void Paint_DrawImage2(const unsigned char* image, UWORD xStart, UWORD yStart, UWORD W_Image, UWORD H_Image) {
  int i, j;
  for (j = 0; j < H_Image; j++) {
    for (i = 0; i < W_Image; i++) {
