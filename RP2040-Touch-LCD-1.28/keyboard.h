...
for (const auto &notification : notifications) {
  ...
  for (int i = 0; i < rowSize; ++i) {
    if(row[i]!=""){
        ...
        if (Touch_CTS816.x_point >= keyStartX-2 && Touch_CTS816.x_point <= keyStartX + keyWidth + 2 && 
            Touch_CTS816.y_point >= keyStartY-2 && Touch_CTS816.y_point <= keyStartY + keyHeight+ 2) {
