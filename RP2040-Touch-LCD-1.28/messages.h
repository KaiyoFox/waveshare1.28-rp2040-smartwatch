...
void messages() {
  ...
  if(tappedNotif==-1){ // && tappedPhoneNumber == ""
    Paint_DrawString_EN(76, 10, "Messages", &Font16, BLUE, WHITE);

    // Display foundContacts
    int contactY = 30;
    for (const auto &contact : foundContacts) {
        Paint_DrawString_EN(50, contactY, contact.front().c_str(), &Font24, BLACK, GREEN);

        if (tap && inTransition==false && pauseRender == false && watchSwipe==false &&
                Touch_CTS816.x_point >= 50 && Touch_CTS816.x_point <= 200 &&
                Touch_CTS816.y_point >= contactY && Touch_CTS816.y_point <= contactY + 28) {
  ...
  int notifPreviewY = 20;
    ...
    // Draw the text
    Paint_DrawString_EN((240 - selectedNotification.back().size()*11) / 2, 2, selectedNotification.back().c_str(), &Font16, BLUE, WHITE);
    ...
    if (!notification.empty() && notification.front() == selectedTitle) {
        if (notification.size() >= 3) {
            ...
            if (!content.empty()) {
                ...
                if (special == "UserSent") {
                    Paint_DrawString_EN((240 - 30) - (11 * content.length()), notifPreviewY - scrollOffset, content.c_str(), &Font16, BLACK, BLUE);
                } else {
